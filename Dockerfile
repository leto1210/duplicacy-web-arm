...
# Vérifiez la valeur de ARCH
RUN echo "ARCH is arm64"

# Vérifiez les valeurs des versions
RUN echo "DUPLICACY_WEB_VERSION is ${DUPLICACY_WEB_VERSION}"
RUN echo "DUPLICACY_VERSION is ${DUPLICACY_VERSION}"

# Télécharger les binaires appropriés pour arm64
RUN echo "Downloading for arm64"; \
    ...
    if [ $? -ne 0 ]; then echo "Failed to download duplicacy_web for arm64" && exit 1; fi; \
    ...
    if [ $? -ne 0 ]; then echo "Failed to download duplicacy for arm64" && exit 1; fi
